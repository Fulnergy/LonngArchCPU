// ============================================================================
// tb_core — core_top 集成测试平台 (带 AXI DRAM 模拟, 支持 burst)
// ============================================================================
//   模拟 64KB DRAM (0x0000~0xFFFF)，BRAM 初始化为 bench_core.mem。
//   通过 AXI 接口与 core_top 通信。支持 burst 读写。10000 周期后 $finish。
// ============================================================================
...
module tb_core;
    ...
    // ============================================================
    // 读通道 (支持 burst: arlen 指定 burst 长度)
    //
    //   IDLE: arready=1. AR握手 → 锁存 addr/id/len → RESP
    //   RESP: 连续返回 arlen+1 拍数据, 每拍 rvalid=1,
    //         地址自增, 末拍 rlast=1.
    // ============================================================
    ...
    reg [ 3:0] r_id_latch;
    reg [ 7:0] r_len;             // 剩余 beat 数 (arlen)
    reg [ 7:0] r_cnt;             // 当前 beat 计数
    ...
    // 写通道 (支持 burst)
    //
    //   IDLE: awready=1, wready=1. AW握手锁存 addr/len.
    //         连续接收 W beats, 每拍写入 DRAM, 地址自增.
    //         wlast=1 → RESP
    //   RESP: bvalid=1. bvalid & bready → IDLE
    // ============================================================
    localparam W_IDLE = 2'd0, W_RESP = 2'd1;
    reg [1:0] w_state;
    reg [31:0] w_addr_latch;
    reg [ 7:0] w_len_latch;
    reg [ 7:0] w_cnt;
    reg        aw_done;
